...
const createMarkSheet = async (accessToken, assessmentId, qa, extractedData) => {
...
const criteria_based_grading = async (studentResponses, questions) => {
    ...
    for (const [studentIndex, responses] of studentResponses.entries()) {
      ...
      // Push the Mail
      const mail = response_object['Please Enter Your Email. This will be used for grading or else your marks will be lost'];
      data.push(mail);
      ...
      for (const [questionIndex, question] of questions.entries()) {
        ...
        for (const [criteriaIndex, val] of criterias.entries()) {
          ...
          const res = await axios.post(BASE_API_URL + "/rag_gemma_assessment", request);
